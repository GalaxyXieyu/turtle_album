# Auto-generated test module for Admin产品管理接口测试
...
@allure.epic('integration_products_admin.yaml')
class TestAdmin产品管理接口测试:
    @classmethod
    def setup_class(cls):
        log.info('========== 开始执行测试用例：test_Admin产品管理接口测试 (测试管理员产品管理接口) ==========')
        ...
        log.info('Setup completed for TestAdmin产品管理接口测试')

    @staticmethod
    def load_test_case_data():
        with open(r'tests/integration_products_admin.yaml', 'r', encoding='utf-8') as file:
            test_case_data = yaml.safe_load(file)['testcase']
        return test_case_data

    @allure.story('Admin产品管理接口测试')
    def test_Admin产品管理接口测试(self):
        log.info('Starting test_Admin产品管理接口测试')
        # Step: setup_admin
        log.info(f'开始执行 step: setup_admin')
        setup_admin = self.steps_dict.get('setup_admin')
        project_config = self.global_vars.get('integration_products_admin.yaml')
        ...
        # Step: step7
        log.info(f'开始执行 step: step7')
        step7 = self.steps_dict.get('step7')
        project_config = self.global_vars.get('integration_products_admin.yaml')
        response = RequestHandler.send_request(
            method=step7['method'],
            url=project_config['host'] + self.VR.process_data(step7['path']),
            headers=self.VR.process_data(step7.get('headers')),
            data=self.VR.process_data(step7.get('data')),
            params=self.VR.process_data(step7.get('params')),
            files=self.VR.process_data(step7.get('files'))
        )
        log.info(f'step7 请求结果为：{response}')
        self.session_vars['step7'] = response
        db_config = project_config.get('mysql')
        AssertHandler().handle_assertion(
            asserts=self.VR.process_data(step7['assert']),
            response=response,
            db_config=db_config
        )


        log.info(f"Test case test_Admin产品管理接口测试 completed.")
