# Auto-generated test module for Settings系统设置接口测试
...
@allure.epic('integration_settings.yaml')
class TestSettings系统设置接口测试:
    @classmethod
    def setup_class(cls):
        log.info('========== 开始执行测试用例：test_Settings系统设置接口测试 (测试系统设置管理接口) ==========')
        ...
        log.info('Setup completed for TestSettings系统设置接口测试')

    @staticmethod
    def load_test_case_data():
        with open(r'tests/integration_settings.yaml', 'r', encoding='utf-8') as file:
            test_case_data = yaml.safe_load(file)['testcase']
        return test_case_data

    @allure.story('Settings系统设置接口测试')
    def test_Settings系统设置接口测试(self):
        log.info('Starting test_Settings系统设置接口测试')
        # Step: setup_admin
        log.info(f'开始执行 step: setup_admin')
        setup_admin = self.steps_dict.get('setup_admin')
        project_config = self.global_vars.get('integration_settings.yaml')
        ...
        log.info(f"Test case test_Settings系统设置接口测试 completed.")
